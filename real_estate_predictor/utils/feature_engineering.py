...
def create_sqft_avg_column(df):
    ...
    """
    df["sqft_avg"] = pd.to_numeric(
        df["sqft"].apply(helper_create_avg_sqft), errors="coerce"
    )
...
def create_bedbathRatio_column(df):
    ...
    df["bedbathRatio"] = _numBedrooms.div(_numBathrooms).replace(
...
def helper_get_unique_ammenities(df, column):
    ...
    if column != "condo_ammenities" and column != "ammenities":
        raise ValueError(
            "incorrect column name, expected condo_ammenities or ammenities"
        )
    ...
    if not ammenities:
        ...
        for type_of_ammenity in available_ammenity_columns:
            ...
            for ammenity in list_of_ammenities:
                df[f"has{ammenity.capitalize()}"] = (
                    df[type_of_ammenity]
                    .apply(helper_find_existence_of_ammenity, args=(ammenity,))
                    .astype(bool)
                )
        ...
        if ammenities != "ammenities" and ammenities != "condo_ammenities":
            raise ValueError(
                "invalid ammenities passed, can only be `ammenities` or `condo_ammenities`"
            )
...
def create_num_ammenities_column(df):
    ...
    df["numCondoAmmenities"] = df["condo_ammenities"].apply(
        helper_calculate_num_ammenities
    )
...
def create_split_postalcode_column(df, num_chars=2):
    ...
    df[f"postal_code_split_{num_chars}"] = df["zip"].apply(
        lambda x: helper_split_postal_code(x, num_chars)
    )
...
def create_dom_column(df):
    ...
    df["daysOnMarket"] = df["daysOnMarket"].div(
        np.timedelta64(1, "D"), fill_value=np.nan
    )
...
def create_previous_month_ppsqft_columns(df):
    ...
    """
    df["avg_soldPrice_ppsqft_currentL1M"] = pd.to_numeric(
        df["avg_soldPrice_currentL1M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_soldPrice_ppsqft_currentL1M"] = pd.to_numeric(
        df["med_soldPrice_currentL1M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["avg_listPrice_ppsqft_currentL1M"] = pd.to_numeric(
        df["avg_listPrice_currentL1M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_listPrice_ppsqft_currentL1M"] = pd.to_numeric(
        df["med_listPrice_currentL1M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )

    df["avg_soldPrice_ppsqft_currentL3M"] = pd.to_numeric(
        df["avg_soldPrice_currentL3M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_soldPrice_ppsqft_currentL3M"] = pd.to_numeric(
        df["med_soldPrice_currentL3M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["avg_listPrice_ppsqft_currentL3M"] = pd.to_numeric(
        df["avg_listPrice_currentL3M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_listPrice_ppsqft_currentL3M"] = pd.to_numeric(
        df["med_listPrice_currentL3M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )

    df["avg_soldPrice_ppsqft_currentL6M"] = pd.to_numeric(
        df["avg_soldPrice_currentL6M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_soldPrice_ppsqft_currentL6M"] = pd.to_numeric(
        df["avg_soldPrice_currentL6M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["avg_listPrice_ppsqft_currentL6M"] = pd.to_numeric(
        df["avg_listPrice_currentL6M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
    )
    df["med_listPrice_ppsqft_currentL6M"] = pd.to_numeric(
        df["avg_listPrice_currentL6M"]
        .div(df["sqft_avg"])
        .replace([np.inf, -np.inf], np.nan),
        errors="coerce",
...
def create_difference_bymonth_columns(df):
    ...
    """
    df["avg_soldPrice_difference_1M_3M"] = (
        df["avg_soldPrice_currentL1M"] - df["avg_soldPrice_currentL3M"]
    )
    df["avg_soldPrice_difference_3M_6M"] = (
        df["avg_soldPrice_currentL3M"] - df["avg_soldPrice_currentL6M"]
    )
    df["avg_listPrice_difference_1M_3M"] = (
        df["avg_listPrice_currentL1M"] - df["avg_listPrice_currentL3M"]
    )
    df["avg_listPrice_difference_3M_6M"] = (
        df["avg_listPrice_currentL3M"] - df["avg_listPrice_currentL6M"]
    )

    df["med_soldPrice_difference_1M_3M"] = (
        df["med_soldPrice_currentL1M"] - df["med_soldPrice_currentL3M"]
    )
    df["med_soldPrice_difference_3M_6M"] = (
        df["med_soldPrice_currentL3M"] - df["med_soldPrice_currentL6M"]
    )
    df["med_listPrice_difference_1M_3M"] = (
        df["med_listPrice_currentL1M"] - df["med_listPrice_currentL3M"]
    )
    df["med_listPrice_difference_3M_6M"] = (
        df["med_listPrice_currentL3M"] - df["med_listPrice_currentL6M"]
    )

    df["count_soldPrice_difference_1M_3M"] = (
        df["count_soldPrice_currentL1M"] - df["count_soldPrice_currentL3M"]
    )
    df["count_soldPrice_difference_3M_6M"] = (
        df["count_soldPrice_currentL3M"] - df["count_soldPrice_currentL6M"]
    )
    df["count_listPrice_difference_1M_3M"] = (
        df["count_listPrice_currentL1M"] - df["count_listPrice_currentL3M"]
    )
    df["count_listPrice_difference_3M_6M"] = (
        df["count_listPrice_currentL3M"] - df["count_listPrice_currentL6M"]
    )
...
def create_ratio_bymonth_columns(df):
    ...
    Assumes the existence of columns:
        ...
        `count_listPrice_currentL*M` where * = 1, 3, 6
    """
    df["avg_soldPrice_ratio_1M_3M"] = (
        df["avg_soldPrice_currentL1M"]
        .div(df["avg_soldPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["avg_soldPrice_ratio_3M_6M"] = (
        df["avg_soldPrice_currentL3M"]
        .div(df["avg_soldPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["avg_listPrice_ratio_1M_3M"] = (
        df["avg_listPrice_currentL1M"]
        .div(df["avg_soldPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["avg_listPrice_ratio_3M_6M"] = (
        df["avg_listPrice_currentL3M"]
        .div(df["avg_soldPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )

    df["med_soldPrice_ratio_1M_3M"] = (
        df["med_soldPrice_currentL1M"]
        .div(df["med_soldPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["med_soldPrice_ratio_3M_6M"] = (
        df["med_soldPrice_currentL3M"]
        .div(df["med_soldPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["med_listPrice_ratio_1M_3M"] = (
        df["med_listPrice_currentL1M"]
        .div(df["med_listPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["med_listPrice_ratio_3M_6M"] = (
        df["med_listPrice_currentL3M"]
        .div(df["med_listPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )

    df["count_soldPrice_ratio_1M_3M"] = (
        df["count_soldPrice_currentL1M"]
        .div(df["count_soldPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["count_soldPrice_ratio_3M_6M"] = (
        df["count_soldPrice_currentL3M"]
        .div(df["count_soldPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["count_listPrice_ratio_1M_3M"] = (
        df["count_listPrice_currentL1M"]
        .div(df["count_listPrice_currentL3M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )
    df["count_listPrice_ratio_3M_6M"] = (
        df["count_listPrice_currentL3M"]
        .div(df["count_listPrice_currentL6M"], fill_value=np.nan)
        .replace([np.inf, -np.inf], np.nan)
    )

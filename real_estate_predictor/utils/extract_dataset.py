...
def extract_raw_data_listings(
    raw_df: pd.DataFrame, inplace=True, verbose=False
) -> pd.DataFrame:
    ...
    assert all(
        [True for col in LISTING_EXPECTED_COLUMNS if col in df.columns]
    ), "some not found in dataset that are expected"
    ...
    datetime_cols = [
        col
        for col in LISTING_COLUMN_TO_DTYPE_MAPPING.keys()
        if LISTING_COLUMN_TO_DTYPE_MAPPING[col] == np.datetime64
    ]
    numerical_cols = [
        col
        for col in LISTING_COLUMN_TO_DTYPE_MAPPING.keys()
        if LISTING_COLUMN_TO_DTYPE_MAPPING[col] == float
    ]
    list_cols = [
        col
        for col in LISTING_COLUMN_TO_DTYPE_MAPPING.keys()
        if LISTING_COLUMN_TO_DTYPE_MAPPING[col] == list
    ]
    str_cols = [
        col
        for col in LISTING_COLUMN_TO_DTYPE_MAPPING.keys()
        if LISTING_COLUMN_TO_DTYPE_MAPPING[col] == str
    ]
    int_cols = [
        col
        for col in LISTING_COLUMN_TO_DTYPE_MAPPING.keys()
        if LISTING_COLUMN_TO_DTYPE_MAPPING[col] == int
    ]
    ...
    if verbose:
        for col in df.columns:
            ...
            print(
                f"Column '{col}' contains the following data types: {unique_types} and ratios:"
            )
...
def extract_neighborhood_data(data: dict, location, bed, type, verbose=False):
    ...
    soldPrice_mth_stats = data["statistics"]["soldPrice"][
        "mth"
    ]  # gives avg, count and med
    listPrice_mth_stats = data["statistics"]["listPrice"][
        "mth"
    ]  # gives avg, count and med
    ...
    for index in soldPrice_mth_stats_df.index:
        soldPrice_mth_stats_df = soldPrice_mth_stats_df.rename(
            index={index: f"{index}_{bed}_{type}_{location}"}
        )
        listPrice_mth_stats_df = listPrice_mth_stats_df.rename(
            index={index: f"{index}_{bed}_{type}_{location}"}
        )
        daysOnMarket_mth_stats_df = daysOnMarket_mth_stats_df.rename(
            index={index: f"{index}_{bed}_{type}_{location}_"}
        )

    available_mth_stats_df = available_mth_stats_df.rename(
        index={"count": f"count_{bed}_{type}_{location}"}
    )

    try:
        soldPrice_mth_stats_df = extract_neighbourhood_df(
            soldPrice_mth_stats_df, "soldPrice"
        )
        listPrice_mth_stats_df = extract_neighbourhood_df(
            listPrice_mth_stats_df, "listPrice"
        )
        available_mth_stats_df = extract_neighbourhood_df(
            available_mth_stats_df, "available"
        )
        daysOnMarket_mth_stats_df = extract_neighbourhood_df(
            daysOnMarket_mth_stats_df, "daysOnMarket"
        )

        merged_inital_df = (
            soldPrice_mth_stats_df.merge(listPrice_mth_stats_df, on="key")
            .merge(available_mth_stats_df, on="key")
            .merge(daysOnMarket_mth_stats_df, on="key")
        )

        merged_inital_df["Date_1M"] = merged_inital_df["key"].apply(
            lambda x: subtract_months(x, 1)
        )
        date_1m_df = merged_inital_df.merge(
            left_on="Date_1M",
            right_on="key",
            suffixes=[None, "L1M"],
            right=merged_inital_df,
            how="left",
        ).drop(
            columns=["keyL1M", "Date_1ML1M", "Date_1M"]
            + [col for col in merged_inital_df.columns if "current" in col]
        )
        merged_inital_df["Date_3M"] = merged_inital_df["key"].apply(
            lambda x: subtract_months(x, 3)
        )
        date_3m_df = merged_inital_df.merge(
            left_on="Date_3M",
            right_on="key",
            suffixes=[None, "L3M"],
            right=merged_inital_df,
            ...
            columns=["keyL3M", "Date_1ML3M", "Date_3ML3M", "Date_1M", "Date_3M"]
            + [col for col in merged_inital_df.columns if "current" in col]
        )
        merged_inital_df["Date_6M"] = merged_inital_df["key"].apply(
            lambda x: subtract_months(x, 6)
        )
        date_6m_df = merged_inital_df.merge(
            left_on="Date_6M",
            right_on="key",
            suffixes=[None, "L6M"],
            right=merged_inital_df,
        ...
        merged_final_df = date_1m_df.merge(date_3m_df, on="key").merge(
            date_6m_df, on="key"
        )
...
def extract_neighbourhood_df(df, metric):
    ...
    df["index"] = (
        ...
        + "_"
        + df["key"].str.split("_").str[3]
    )
    df["new_index"] = df["index"] + "_" + df["Date"]
    df["metric"] = df["key"].str.split("_").str[0]
    metric_columns = ["key"] + [
        f"{agg}_{metric}_current" for agg in sorted(set(df["metric"].values))
    ]
...
def convert_col_dtype(
    df: pd.DataFrame, columns: list, convert_to_type: str, errors: str = "coerce"
):
    """
    ...
    if convert_to_type == "str":
        ...
        raise ValueError(
            f"Unexpected convert_to_type {convert_to_type}"

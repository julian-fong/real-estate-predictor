...
class DataCleaner(FunctionLogger):
    ...
    def remove_duplicates(
        self, columns, ignore_index=True, strategy="subset", df=None, defer=False
    ):
        """
    ...
    def handle_missing_values(
        self, strategy, columns=None, threshold=None, df=None, defer=False
    ):
        """
    ...
    @FunctionLogger.log_function_call
    def filter_rows_by_threshold(
        self, columns, strategy, threshold, df=None, defer=False
    ):
        """
    ...
    @FunctionLogger.log_function_call
    def replace_values_via_mask(
        self, columns, strategy, threshold, replacement=np.nan, df=None, defer=False
    ):
        """
    ...
    @FunctionLogger.log_function_call
    def replace_value(
        self, columns, value, replacement, df: pd.DataFrame = None, defer=False
    ):
        if not df:
            df = self.df
        if isinstance(columns, str):
...
class Processor:
    ...
    def impute_numerical(self, df=None, strategy=None, columns=None, f=None):
        ...
        self.numerical_imputer.append(
            {"name": f"num_imputer{num}", "pipeline": f, "columns": columns}
        )
    ...
    def transform_numerical(self, df=None, strategy=None, columns=None, f=None):
        ...
        self.numerical_transformer.append(
            {"name": f"num_transformer_{num}", "pipeline": f, "columns": columns}
        )
    ...
    def impute_categorical(self, df=None, strategy=None, columns=None, f=None):
        ...
        self.categorical_imputer.append(
            {"name": f"cate_imputer_{num}", "pipeline": f, "columns": columns}
        )
    ...
    def encode_categorical(self, df=None, strategy=None, columns=None, f=None):
        ...
        self.categorical_encoder.append(
            {"name": f"cate_encoder_{num}", "pipeline": f, "columns": columns}
        )
    ...
    def apply_transformer(self):
        ...
        self.preprocessor = ColumnTransformer(
            transformers=self.transformers, remainder="passthrough"
        )
            ...
            for col in numerical_imputer["columns"]:
                ...
                for numerical_transformer in self.numerical_transformer:
                    if col in numerical_transformer["columns"]:
                        ...
                        transformer.steps.append(
                            (
                                f"numerical_transformer_{col}",
                                numerical_transformer["pipeline"],
                            )
                        )
                self.transformers.append(
                    (f"transformer_{len(self.transformers)+1}", transformer, [col])
                )

        # add in the remaining transformers inside numerical_transformer that were not seen in imputer
        for numerical_transformer in self.numerical_transformer:
            if not self._check_if_subset(
                numerical_transformer["columns"], seen_columns
            ):
                transformer = Pipeline(steps=[])
                transformer.steps.append(
                    (
                        f"numerical_transformer_{str(numerical_transformer['columns'])}",
                        numerical_transformer["pipeline"],
                    )
                )
                self.transformers.append(
                    (
                        f"transformer_{len(self.transformers)+1}",
                        transformer,
                        numerical_transformer["columns"],
                    )
                )
    ...
    def fit(self, X):
        if not self.preprocessor:
            raise ValueError(
                "ColumnTransformer not initialized, please run `apply_column_transformer`"
            )
    ...
    def transform(self, X):
        ...
        for processor in self.preprocessor.named_transformers_.keys():
            if isinstance(self.preprocessor.named_transformers_[processor], Pipeline):
                ...
                categorical_names = [
                    t[0] for t in self.preprocessor.named_transformers_[processor].steps
                ]
                if "categorical_encoder" in categorical_names:
                    feature_names += (
                        self.preprocessor.named_transformers_[processor]
                        .get_feature_names_out()
                        .tolist()
                    )
                    categorical_columns_in += self.preprocessor.named_transformers_[
                        processor
                    ].feature_names_in_.tolist()
                else:
                    feature_names += self.preprocessor.named_transformers_[
                        processor
                    ].feature_names_in_.tolist()
            else:
                feature_names += self.preprocessor.named_transformers_[
...
class FeatureEngineering(FunctionLogger):
    ...
    def create_features_old(self, columns=None, ignore_features=None):
        ...
        if self.passed_columns:
            warnings.warn(
                "Columns have already been passed once, overriding previous set columns"
            )
        ...
        for feature in FEATURE_ENGINEERING_PARAMETERS.keys():
            if feature in columns and feature not in ignore_features:
                ...
                if not all(
                    [
                        True if col in self.df.columns else False
                        for col in column_dependencies
                    ]
                ):
                    warnings.warn(
                        f"Not all column dependencies in {column_dependencies} for feature {feature} not found, skipping"
                    )
                else:
                    self.df = f(self.df)
    ...
    def _evaluate_feature(self, list_of_dependencies):
        if not all(
            [True if col in self.df.columns else False for col in list_of_dependencies]
        ):
            return True
        else:
            return False

...
class Dataset:
    ...
    def get_listings_dataset(self, start_date, end_date, save=False, verbose=False):
        ...
        for i in range(1, numPages + 1):
            _, _, data = retrieve_repliers_listing_request(
                start_date, end_date, i, LISTING_PARAMETERS, verbose=verbose
            )
    ...
    def get_neighbourhood_dataset(
        self, start_date, end_date, neighborhoods=None, save=False, verbose=False
    ):
        ...
        for bed in NEIGHBOURHOOD_NUMBEDROOMS:
            for type in NEIGHBOURHOOD_TYPES:
                for location in self._neighborhoods:
                    ...
                    neighborhood_data = extract_neighborhood_data(
                        data, location, bed, type, verbose
                    )

                    if (
                        isinstance(neighborhood_data, pd.DataFrame)
                        and len(neighborhood_data) > 0
                    ):
